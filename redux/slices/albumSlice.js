  ...
  reducers: {
    ...
    photoUploadCompleted: (state, action) => {
      state.albums.forEach(item => {
        if (item.AlbumId === action.payload.albumId) {
          item.photos.unshift(action.payload.photo)
        }
      })
      state.currentAlbum.photos.forEach(item => {
        if (item.id === action.payload.photo.localId) {
    ...
    fetchAlbums: (state, action) => {
      ...
      state.currentAlbum = {...action.payload[0], photos: []}
    },
